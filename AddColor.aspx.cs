...
namespace UsingBootstrapWebApplication
{
    public partial class AddColor : System.Web.UI.Page
    {
        ...
        private void BindQuantityRepeater()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("select * from ProductColorTable", con))
                {
        ...
        protected void QuantityAddButton_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("insert into ProductColorTable values('" + ProductQuantityTextBox.Text + "','" + ProductDropDownId.SelectedItem.Value + "')", con);
                con.Open();
                cmd.ExecuteNonQuery();
